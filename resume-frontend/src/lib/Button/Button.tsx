...
const StyledButton = styled.button<StyledProps>`
  ...
  border: 1px solid
    ${(p) => returnColor(p.variant, p.fill, "default", "border")};
  ...
  ${(p) =>
    ...
    `
        opacity: 1;
        background: ${
          p.fill === "ghost" ? "hsla(0, 0%, 100%, 0)" : "var(--sand3)"
        };
...
const Inner = styled.span<StyledProps>`
  ...
  i {
    ...
    color: ${(p) =>
      p.icon
        ? undefined
        : returnColor(p.variant, p.fill, "default", "iconColor")};
  }
  ...
  (
    ...
    return (
      ...
      >
        <>
          {loading && (
            <Spinner {...styledProps} className="ph-bold ph-circle-notch" />
          )}
          <Inner {...styledProps}>
            {icon ? (
              <i className={icon} />
