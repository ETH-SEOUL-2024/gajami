...
const onCreateAccount = async ({
  ...
  // todo: resume signup
  await userSignup(email, res.near_account_id, setStatusMessage);
  window.location.replace(parsedUrl.href);
};

// https://near-test-fastauth-issuer.firebaseapp.com/__/auth/action?apiKey=AIzaSyBY-gf5WM4PoA4FPzOL7FuiQoTdxTuRSNE&mode=signIn&oobCode=49gDAbslJsc5NhP6HXuSFRs7Q-0HIzw8CeX9vZnhRsMAAAGOdVsCew&continueUrl=http://localhost:3000/auth-callback?accountId%3Djong9712.testnet&lang=en
export const onSignIn = async ({
  ...
  const recoveryPK = await window.fastAuthController.getUserCredential(accessToken);
  console.log(`Recovering account !!!:${recoveryPK}`);
  ...
  return (window as any).fastAuthController.signAndSendActionsWithRecoveryKey({
    ...
    .then(async (res) => {
      ...
      if (failure?.ActionError?.kind?.LackBalanceForState) {
        ...
        // todo: resume login
        await userLogin(email, accountIds[0], setStatusMessage);
    ...
    const signInProcess = async () => {
      if (isSignInWithEmailLink(firebaseAuth, window.location.href)) {
        ...
        try {
          ...
          setStatusMessage(`callback isRecovery: ${isRecovery}, so calling ${(isRecovery ? 'onSignIn' : 'onCreateAccount')}`);
